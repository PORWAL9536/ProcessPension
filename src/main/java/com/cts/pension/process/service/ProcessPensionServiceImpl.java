...
/**
 * @author vivek Implementing the service class to calculate the pension amount
 */
@Service
public class ProcessPensionServiceImpl implements ProcessPensionService {
...

	{
		...
		LOGGER.debug("Adddhaaaaaaaaaaaar" + pensionerInput.getAadharNumber());

		PensionerDetail pensionerDetail = null;

		try {
			pensionerDetail = pensionerDeatailFeignClient.getPensionerDetailByAadhaar(token,
					pensionerInput.getAadharNumber());
			LOGGER.debug("pensionerDetail" + pensionerDetail);
		...
		if (pensionerInput.getAadharNumber() == pensionerDetail.getAadharNumber()
			...
			/*
			 * if(pensionerInput.getPensionType().equalsIgnoreCase("self")) { pensionAmount
			 * = 0.8*salary + allowances; } else
			 * if(pensionerInput.getPensionType().equalsIgnoreCase("family")) {
			 * pensionAmount = 0.5 * salary + allowances; }
			 */
			String var = pensionerInput.getPensionType();
			switch (var) {
			case "self":
	...
	@Override
	public ProcessPensionResponse getCode(String token, ProcessPensionInput processPensionInput)
